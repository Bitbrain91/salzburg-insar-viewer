...
- `frontend/`: React + Vite UI. Source lives in `frontend/src/` with components in `frontend/src/components/` and shared state in `frontend/src/lib/`.
  - `components/layout/`: resizable app shell (react-resizable-panels), map overlays (ActiveRunChip, MapLegend), timeseries dock.
  - `components/runs/`: run manager (run cards, filters, rename, new-run form) and the run detail shown in the inspector.
...
- `explainers/`: separate Vite/React explainer app (pipeline journey + silver-ground-truth explainer + the "InSAR data-point genesis" explainer). Chapter registries and the number contracts live in `explainers/src/content/` (`facts.ts`, `silverFacts.ts`, `insarFacts.ts`); update them in the same ticket as the mirrored pipeline/corpus changes — for `insarFacts.ts` whenever the handbook facts or the Salzburg dataset figures change.
